...
const d = computed(() => { return `M 0 0 q ${dx.value / 2} 0 ${dx.value} ${dy.value} q ${dx.value / 2} ${dy.value} ${dx.value} ${dy.value}` })
</script>
...
<template>
  <Card>
    ...
    <svg ref="refSVGContainer" :width="width" :height="height">
      ...
      />
      <circle r="5" cx="0" cy="0" fill="yellow" stroke="gray 2px" />
